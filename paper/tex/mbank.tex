...
Section~\ref{sec:validation} will be devoted to the validation of the performance of the code. We will assess the accuracy of the metric approximation and of the tamplate placing methods we implemented. We also compare our code with the widely used stochastic placement code \texttt{sbank}.
To demostrate the capabilities of \mbank, we will employ our code to generate three large banks of exotic regions of parameter space: a bank of precessing signals, a bank of Intermediate Mass Black Hole (IMBH) with Higher Order Modes (HM) and a bank of eccentric signals. This will be the topic of sec.~\ref{sec:bank_generation}.
...
Of course, based on the scope of the search, the signal space can be further reduced to target a limited interesting sub-space. Moreover, in some situation is might be possible to maximize analitically the likelihood over certain quantities, effectively reducing the dimensionality of the manifold.
...
The expression above is a measure of the detection statistics $\Lambda(\theta_1)$ that is measured when a signal $h(t; \theta_2)$ is present in the data.
...
The total number of templates $N_{templates}$ to sample is set by the equation
...
We note that $N_{\text{templates}}$ grow with the number of tiles until a certain upper limit. Indeed we expect that an incresing number of tiles provides a better estimation of the overall volume of the space until the estimation converges to the actual value in the limit of infinite tiles. 
...
As a finale note, we recommend to use, due to its speed and precision, the {\it random} method, whenever it gives good performance. In the cases where it should fails, the method {\it stochastic} or even {\it random-stochastic} are very likely to give the performance required.
...
%	 	60166 & 15549 \\
	 ...
	 The comparison are made on a machine with CPU Intel(R) Xeon(R) CPU E5-2630 installed at CIT cluster \stefano{How do I reference CIT?}.
	 }
...
We presented a novel method to generate template banks to cover a high dimensional manifold of (possibly) precessing/HM/eccentric Binary Black Holes signals.
...
To demostrate the capabilities of our code, we generated three banks covering some interesting and up to now unexplored regions of the parameter space. We found the \mbank is able to provid a faithfully coverage.
...
\stefano{Maybe this last remark can be made more precise/longer?}
