...
def compute_metric_injections_match(injs, bank, tiling, N_neigh_templates = 10, verbose = True):
	...
	for i in inj_iter:
		...
		#FIXME: einsum is super super slow!! Please, do this with choelsky and matmul!! It will go faster!!
...
def place_random(dist, t_obj, N_points, tolerance = 0.01):
	...
	for _ in it:
		...
		dist_ = np.sum(np.square(np.matmul(diff, L_t)), axis =1) #(N,) #This is the bottleneck of the computation, as it should be
		...
		new_templates.append(np.array(point))
		del point
