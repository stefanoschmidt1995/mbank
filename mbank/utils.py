...
####################################################################################################################
def ray_compute_injections_match(inj_dict, templates, metric_obj, antenna_patterns = None, mchirp_window = 0.1, symphony_match = False, max_jobs = 8, verbose = True):
	"""
	...
	----------
		...
		antenna_patterns: tuple
			Tuple of float/:class:`~numpy:numpy.ndarray` (:math:`F_+`, :math:`F_\\times`) for the two antenna patterns. They have the same meaning as in :meth:`mbank.metric.cbc_metric.match`. If ``symphony = True``, it cannot be ``None``.
	...
	for id_, i in enumerate(range(0, inj_dict['metric_match'].shape[0], n_injs_per_job)):
		inj_dict_ray_list.append( _compute_injections_match_ray.remote(i, i+n_injs_per_job, inj_dict,
			templates, metric_obj, antenna_patterns, mchirp_window, symphony_match, id_, verbose))
...
@ray.remote
def _compute_injections_match_ray(start_id, end_id, inj_dict, templates, metric_obj, antenna_patterns = None, mchirp_window = 0.1, symphony_match = False, worker_id = 0, verbose = True):
	"""
	...
	if antenna_patterns is not None:
		...
		if not isinstance(F_p, (int, float)):
			F_p, F_c = F_p[start_id:end_id], F_c[start_id:end_id]
		antenna_patterns = (F_p, F_c)
	
	return compute_injections_match(local_dict, templates, metric_obj, antenna_patterns, mchirp_window, symphony_match, worker_id, verbose)

def compute_injections_match(inj_dict, templates, metric_obj, antenna_patterns = None, mchirp_window = 0.1, symphony_match = False, worker_id = None, verbose = True):
	"""
	...
	if antenna_patterns is not None:
		...
		if symphony_match: raise ValueError("The antenna patterns must be given if the symphony match is used!")
...
def compute_injections_metric_match(injs, bank, tiling, verbose = True):
	...
	-------
		out_dict: dict
			...
			- ``match``: match of the closest template (filled by function :func:`compute_injections_match`)
			- ``id_match``: index of the closest template (filled by function :func:`compute_injections_match`)
