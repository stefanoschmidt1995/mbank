...
def place_random_flow(minimum_match, tiling, N_points, tolerance = 0.01, verbose = True):
	"""
	```WORK IN PROGRESS```
	Given a tiling object, it covers the volume with points and covers them with templates.
	It follows `2202.09380 <https://arxiv.org/abs/2202.09380>`_
	It uses normalizing flow to interpolate the metric
	...
	if verbose: it = tqdm(dummy_iterator(), desc = bar_str.format(N_points -len(livepoints), N_points, len(new_templates)), leave = True)
		...
		#initializing flow
		# super super ugly!!!
	from .flow.flowmodel import GW_Flow
	import sys
	sys.path.insert(0,'/home/stefano/Dropbox/Stefano/PhD/mbank/flow_stuff') #super ugly!!
	from build_flow import Std3DTransform
	from nflows.distributions.normal import StandardNormal
	import torch
	
	flow = GW_Flow(transform=Std3DTransform(), distribution=StandardNormal(shape=[3]))
	flow.load_state_dict(torch.load('/home/stefano/Dropbox/Stefano/PhD/mbank/flow_stuff/standard_flow_3D/weights'))
	...
	for _ in it:
		...
		dist = flow.get_distance(livepoints, point, tiling[id_].center, metric)
		quit()
