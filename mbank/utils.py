...
def ray_compute_injections_match(inj_dict, templates, metric_obj, N_neigh_templates = 10, symphony_match = False, max_jobs = 8, cache = True):
	"""
...
def compute_injections_match(inj_dict, templates, metric_obj, N_neigh_templates = 50, symphony_match = False, cache = True, worker_id = None):
...
####################################################################################################################	
#@do_profile(follow=[])
def compute_metric_injections_match(injs, bank, tiling, N_neigh_templates = 100, verbose = True):
	...
	for i in inj_iter:
		...
		#match_i_new = 1- np.sum(np.square(np.matmul(diff[id_diff_ok], L_t)), axis =1)
		
		#match_i = 1 - np.einsum('ij, jk, ik -> i', diff[id_diff_ok], metric, diff[id_diff_ok]) #slower version
		...
		out_dict['id_metric_match'][i] = out_dict['id_match_neig'][i, int(template_dist)]
		out_dict['metric_match'][i] = out_dict['match_neig'][i, int(template_dist)]
...
def place_stochastically_global_stop(minimum_match, tiling, empty_iterations = 200, seed_bank = None, verbose = True):
	"""
	...
	try:
		for _ in t_:
			...
			if verbose and i%200==0:t_.set_description("Templates added {} ({}/{} empty iterations)".format(new_templates.shape[0], int(nothing_new), int(empty_iterations)))
...
def place_stochastically(minimum_match, tiling, empty_iterations = 200, seed_bank = None, verbose = True):
	"""
	Place templates with a stochastic placing algorithm
	It iteratively proposes a new template to add to the bank. The proposal is accepted if the match of the proposal with the previously placed templates is smaller than ``minimum_match``. Within each tile, the iteration goes on until no template is found to have a distance smaller than the given threshold ``minimum_match``.
	It can start from a given set of templates.
	...
	try:
		...
		for _ in t_:
			...
			if np.any(where_to_remove):
				ids_ = np.where(where_to_remove)
				tiles_to_use = np.delete(tiles_to_use, ids_)
				nothing_new = np.delete(nothing_new, ids_)
