...
GW searches for signals from compact binary mergers traditionally utilize the method of matched-filtering with a template bank of model waveforms~\cite{Owen:1998dk}. Accurate template banks that cover the parameter space of interest can be computationally expensive to create, but are crucial for ensuring sensitivity of state-of-the-art searches.
...
When searching for a BBH signal in GW data, it is custom to use a frequentist detection statistics $\Lambda$ \cite{Creighton_book, Maggiore:2007ulw, Harry:2016ijz, PhysRevD.97.023004}.
...
The exact expression of $p(s = n+h(\theta)|d)$ and $p(s = n|d)$ is obtained under the assumption of stationary Gaussian noise and it depends on the quantities $\scalar{s}{h}$ and $\scalar{h}{h}$, where $\scalar{\cdot}{\cdot}$ is a suitable scalar product (see below).
...
We will devote the rest of this section to detail the steps above.
...
%As a final remark, we note that other definitions for the match are possible (see e.g. \cite{Harry:2016ijz,PhysRevD.97.023004}). These are motivated by different expressions for the detection statistics, which depends on the assumptions made on the signal (i.e. precessing or eccentric signals or with Higher Order Modes). Here we limit to use the ``vanilla" expression~\eqref{eq:match} for aligned spins BBH (for which $\tilde{h}_+ \propto i \tilde{h}_\times$), even for cases where it does not hold. We expect the error introduced by this to be negligible.
...
\end{equation}
where $R_{[\theta_\text{min}, \theta_\text{max}]}$ is an hyper-rectangles with extrema $\theta_{\text{min/max}}$ and where $M$ is the metric evaluated Eq.~\eqref{eq:metric_expression} at the center of the rectangle $\frac{\theta_\text{min}+\theta_\text{max}}{2}$.

Taking inspiration from \cite{Hanna:2022zpk}, we use the function \texttt{SplitTile} to split a tile $\mathcal{T}$ in 3 tiles $\mathcal{T}_\text{left}$, $\mathcal{T}_\text{center}$, $\mathcal{T}_\text{right}$.
The three resulting tiles are further split if the metric determinants $\text{det}M_\text{right}, \text{det}M_\text{left}$ of the left and right tile satisfy:
...
The tiling defines a {\it fast} approximation $M^{\text{tiling}}_{ij}(\theta)$ to the true metric $M_{ij}(\theta)$ Eq.~\eqref{eq:metric_expression}:
...
Moreover, a tiling provides an approximation $p^{\text{tiling}}$ to the uniform probability distribution Eq.~\eqref{eq:pdf_uniform}:
...
A normalizing flow model \cite{norm_flow, Kobyzev_2021, nflows_paper, Papamakarios_thesis} is a machine learning model widely used to reproduce complicated probability distributions.
...
\end{equation}
%
In other worlds, a normalizing flow defines a parametric representation of a generic probability distribution $p(\theta)$, obtained by change of variables
...
The weights $W$ of the flow are set by miniziming the loss function $\mathcal{L}_\phi(W)$, defined as the forward Kullback–Leibler (KL) divergence between the target distribution $p(\theta)$ and the one defined by the flow Eq.~\eqref{eq:p_flow}:
\begin{align}
...
Next in Sec.~\ref{sec:placing_accuracy}, we compare the different placing methods and assess their performance on several tiling.
...
In much of what follows we will need to measure the coverage of a bank. To do so, we follow a very standard procedure. We randomly extract a number of signals (usually called injections)\footnote{
...
The metric approximation consistently {\it underestimates} the actual match and provides a lower limit to the true match.
