...
def compute_metric_injections_match(injs, bank, tiling, N_neigh_templates = 10, verbose = True):
	...
	for i in inj_iter:
		...
		out_dict['id_tile'][i] = np.argmin([R.min_distance_point(injs[i]) for R, _ in tiling ])
	...
	for id_ in it:
		...
		while nothing_new < empty_iteration:
			...
			#ids_kill = np.where(dist_< dist_sq)[0]
				...
				#measuring metric match between livepoints and proposal
				#Limiting the match to small distances (where we expect the metric approximation to hold)
			ids_test = np.where(np.all(np.abs(diff) < 1, axis =1))[0] #(N',)
			dist_sq_ = np.einsum('ij,jk,ik->i', diff[ids_test,:], metric, diff[ids_test,:]) #(N',)
			ids_kill = np.where(dist_sq_ < dist_sq)[0]
