...
def compute_injections_match(inj_dict, templates, metric_obj, N_neigh_templates = 10):
	"""
	...
	"""
	#TODO: this should run in parallel, somehow
	...
	for i in tqdm(range(injs.shape[0]), desc = 'Computing the match: loop on the injections'):
		...
		#metric_obj.WF_match(template_WF, inj_WF) #(N_neigh_templates,)
			...
			#updating the dict
		true_match = metric_obj.match(template_, injs[i])
...
def create_mesh(dist, tile, coarse_boundaries = None):
	...
	for d in range(boundaries_prime.shape[1]):
		...
		#grid_d = [np.arange(boundaries_prime[0,d], boundaries_prime[1,d], dist)[1:]]
		#if len(grid_d[0]) == 0: grid_d = [[center_prime[d]]]
		
		grid_d = np.concatenate(grid_d)
		#print(d, grid_d, boundaries_prime[:,d], dist)
	...
	for id_random in where_random:
		mesh[:,id_random] = np.random.uniform(boundaries[0,id_random], boundaries[1,id_random], (mesh.shape[0], ))
