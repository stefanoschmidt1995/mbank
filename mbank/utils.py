...
def ray_compute_injections_match(inj_dict, templates, metric_obj, N_neigh_templates = 10, symphony_match = False, cache = True):
	...
	Given an injection dictionary, generated by ``compute_metric_injections_match_bruteforce()`` it computes the actual match (without the metric approximation) between injections and templates. It updates ``inj_dict`` with the new computed results.
...
def compute_metric_injections_match_bruteforce(injs, bank, tiling, N_neigh_templates = 10, verbose = True):
	...
	if injs.shape[1] != bank.D:
		...
		#tile for each injections (to be added to the out_dict)
	dist_t = []
	...
	N_argpartiton = 20000
			...
			#tile for each template (might be painful!!)
	dist_template = []
	
	if verbose: t_iter = tqdm(tiling, desc='Identifying a tile for each template', leave = True)
	else: t_iter = tiling
	
	for t in t_iter:
		dist_template.append( t[0].min_distance_point(bank.templates) ) #(N_templates,)
	dist_template = np.stack(dist_template, axis = 1) #(N_templates, N_tiles)
	id_templates = np.argmin(dist_template, axis = 1) #(N_templates,)
	del dist_template
	metric_array = np.array([m for _, m in tiling]) #(N_tiles, D,D)
	
		#loops on the injections
	if verbose: inj_iter = tqdm(range(injs.shape[0]), desc = 'Evaluating brute force metric match for injections', leave = True)
...
def place_stochastically(avg_dist, t_obj, bank, empty_iterations = 200, seed_bank = None):
	...
	The match of the proposals is computed with `compute_metric_injections_match_bruteforce`.
...
def place_random(dist, tile_obj, N_points, empty_iteration = 50):
	"""
	...
	"""
	dist_sq = dist**2
	livepoints = tile_obj.sample_from_tiling(N_points) #(N_points, D)
	
		#ordering the tile by volume in ascending order...
	#_, vols = tile_obj.compute_volume()
	volume_factors = [np.sqrt(np.abs(np.linalg.det(metric))) for _, metric in tile_obj]
	
	ids_sort = np.argsort(volume_factors)#[::-1]
	...
	for id_ in it:
		...
		nothing_new = 0
		while nothing_new < empty_iteration:
			nothing_new += 1
