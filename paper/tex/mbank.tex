...
Metric fails when the match is bad behaved:

\begin{itemize}
	\item banana shape for the match
	\item one dimension is not important (metric is elongated!)
\end{itemize}
...
Rethink the plot that you want to show here: maybe the fraction of ellipse volume that lies inside the actual region $\{MM<0.97\}$?
\begin{figure}[t]
...
Histogram with the actual match for points with a constant metric match. The study is done with the \textit{parabolic fit} metric.

\subsection{Tiling accuracy} \label{sec:tiling_accuracy}
How much the metric is constant within each tile?
\stefano{Do we want this section? Soumen suggested this but the results don't look promising and as long as the bank is fine we don't care...}
...
\begin{table*}[t!]
	...
	\setlength\extrarowheight{1pt}
	 ...
	 \hline
	 \multicolumn{1}{c}{\phantom{Bank name}} & \multicolumn{1}{c}{\textbf{Ranges}} & \multicolumn{1}{c}{\textbf{Size}}  & \multicolumn{1}{c}{\textbf{Effectualness}} \\
	 \hline
	 Nonspinning & \begin{tabular}{@{}l@{}} $M\in [30,50] M_\odot$ \\ $q\in [1,5]$  \\ $f\in [15,1024] Hz$ \\ \end{tabular}  &
	 	\begin{tabular}{@{}l@{}} \texttt{sbank}: 396 \\ \texttt{mbank}: 442 \end{tabular} & ----  \\
	 \cdashline{1-4}
	 Aligned spin & \begin{tabular}{@{}l@{}} $M\in [30,50]$ \\ $q\in [1,5]$ \\ $s_{1z}, s_{2z}\in [-0.99,0.99]$ \\ $f\in [15,1024] Hz\end{tabular}  &
	 	\begin{tabular}{@{}l@{}} \texttt{sbank}: 3275 \\ \texttt{mbank}: 4108 \end{tabular} & ---- \\
	 \cdashline{1-4}
	 GstLAL O3 bank & \begin{tabular}{@{}l@{}} $M\in [30,100]$ \\ $q\in [1,10]$ \\ $s_{1z}, s_{2z}\in [-0.99,0.99]$ \\ $f\in [15,1024] Hz \end{tabular}  &
	 	\begin{tabular}{@{}l@{}} \texttt{sbank}: ?? \\ \texttt{mbank}: ?? \end{tabular} & ---- \\
	 \hline
	 \end{tabular}
	 \caption{Comparison between the performance of  \texttt{mbank} and  \texttt{sbank} on 3 chosen banks. The details of the injection studies are shown in figure~\ref{fig:sbank_comparison}}
...
IMRPhenomPv2/XP (high mass)

Compare the metric slices with the NP case

\subsection{An aligned spin HM bank}
IMRPhenomXPHM

\subsection{An eccentric nonspinning bank}
EccentricFD (low mass)
...
\begin{table*}[t!]
	...
	\setlength\extrarowheight{1pt}
	 ...
	 \hline
	 \multicolumn{1}{c}{\phantom{Name}} & \multicolumn{1}{c}{\textbf{Ranges}} & \multicolumn{1}{c}{\textbf{Setting}} & \multicolumn{1}{c}{\textbf{Size}}  & \multicolumn{1}{c}{\textbf{Effectualness}} \\
	 \hline
	 Precessing & \begin{tabular}{@{}l@{}} $M\in [25,100] M_\odot$ \\ $q\in [1,5]$  \\ $s_1\in [0,0.99]$ \\$\theta_1\in [0, \pi]$ \\ \end{tabular}  &
	 \begin{tabular}{@{}l@{}} IMRPhenomXP \\ $\epsilon = 0.1$ \\ \texttt{max-depth}: 10 \\ \end{tabular}  &
	 \begin{tabular}{@{}l@{}} $N_{templates} = 54750$ \\ $N_{tiles} = 33774$ \\ \end{tabular} &
	 	----  \\
	 \cdashline{1-5}
	 Aligned spins HM & \begin{tabular}{@{}l@{}} $M\in [30,50] M_\odot$ \\ $q\in [1,5]$  \\ $s_{1z}, s_{2z}\in [-0.99,0.99]$ \\ \end{tabular}  &
	 	 \begin{tabular}{@{}l@{}} IMRPhenomXPHM \\ $\epsilon = ??$ \\ \end{tabular}  &
	 	\begin{tabular}{@{}l@{}} $N_{templates}$ \\ $N_{tiles} = ??$ \\ \end{tabular} &
	 	----  \\
	 \cdashline{1-5}
	 Nonspinning eccentric & \begin{tabular}{@{}l@{}} $M\in [10,75] M_\odot$ \\ $q\in [1,5]$ \\ $e \in [0,0.3]$  \end{tabular}  &
	 	 \begin{tabular}{@{}l@{}} EccentricFD \\ $\epsilon = 0.1$ \\ \texttt{max-depth}: 9 \\  \end{tabular}  &
	 	\begin{tabular}{@{}l@{}} $N_{templates} = 154472$ \\ $N_{tiles} = 4238$ \\ \end{tabular} &
	 	----  \\
	 \hline
	 \end{tabular}
	 \caption{Summary of three use case banks generate with \texttt{mbank}. For each bank generated, we report the variable being sampled and their ranges. We also report the hyperparameters $\epsilon$ used for the tiling. Every bank is generated within a frequency range of $[15,1024] Hz$.	 }
 	 \label{tab:sbank_comparison}
\end{table*}


\begin{figure}[t]
	\includegraphics{bank_hist_Precessing}\hfill
	\includegraphics{bank_hist_Aligned_spins_HM}\hfill
	\includegraphics{bank_hist_Nonspinning_eccentric}
	\caption{Histograms for the use case banks!}}
	\label{fig:bank_hist}
\end{figure}
...
\begin{figure}[t]
	...
	\caption{Injections true vs recovered for case study banks}}
