...
####################################################################################################################
def ray_compute_injections_match(inj_dict, templates, metric_obj, mchirp_window = 0.1, symphony_match = False, max_jobs = 8, cache = True):
	"""
	...
	for id_, i in enumerate(range(0, inj_dict['metric_match'].shape[0], n_injs_per_job)):
		inj_dict_ray_list.append( _compute_injections_match_ray.remote(i, i+n_injs_per_job, inj_dict,
			templates, metric_obj, mchirp_window, symphony_match, cache, id_))
...
@ray.remote
def _compute_injections_match_ray(start_id, end_id, inj_dict, templates, metric_obj, mchirp_window = 0.1, symphony_match = False, cache = True, worker_id = 0):
	"""
	...
	return compute_injections_match(local_dict, templates, metric_obj, mchirp_window, symphony_match, cache, worker_id)

def compute_injections_match_slow(inj_dict, templates, metric_obj, mchirp_window = 0.1, symphony_match = False, cache = True, worker_id = None):
	"""
	**DEPRECATED**
	
	Given an injection dictionary, generated by :func:`compute_injections_metric_match` it computes the match (not the metric approximation) between injections and templates. It updates ``inj_dict`` with the new computed results.
	The injections are generic (not necessarly projected on the bank submanifold).
	...
	for i in tqdm(range(injs.shape[0]), desc = desc, leave = True):
		...
		#ids_close = np.where(relative_diff<mchirp_window)[0]
		N_max = (relative_diff<mchirp_window).sum()
		ids_close = np.argsort(relative_diff)

		max_match = 0.
		match_list = []
		
			#We explore the space carefully and if we find a point which has a match high enough, we keep it without further investigations
		for j in range(0, N_max, 200):
			ids_close_ = ids_close[j:j+200]
			matches = metric_obj.match(templates[ids_close_], injs[i], symphony = symphony_match, overlap = False)

			match_list.extend(matches)
			
			if max_match<np.max(matches): max_match = np.max(matches)
			#if max_match > 0.97: break
		
		matches = np.array(match_list)
		ids_close = ids_close[:j+200]
		
		ids_max = np.argmax(matches)

			#updating the dict
		#inj_dict['match_list'].append(list(matches)) 
		#inj_dict['id_match_list'].append(list(ids_close))
		inj_dict['id_match'][i] = ids_close[ids_max]
		inj_dict['match'][i] = matches[ids_max]
...
def compute_injections_match(inj_dict, templates, metric_obj, mchirp_window = 0.1, symphony_match = False, cache = True, worker_id = None):
	"""
	Given an injection dictionary, generated by :func:`compute_injections_metric_match` it computes the actual match (not the metric approximation) between injections and templates. It updates the ``match`` and ``id_match`` entries of ``inj_dict`` with the newly computed match values.
	The injections are generic (not necessarly projected on the bank submanifold).
	...
	----------
		...
		templates: :class:`~numpy:numpy.ndarray`
			An array with the templates. They should have the same layout as lal (given by :func:`mbank.handlers.variable_handler.get_BBH_components`)
	...
	if metric_obj.variable_format != 'm1m2_fullspins_emeanano_iotaphi':
		...
		#allocating memory for the match
	inj_dict['id_match'] = np.zeros(inj_dict['id_metric_match'].shape, dtype=inj_dict['id_metric_match'].dtype)
	inj_dict['match'] = np.zeros(inj_dict['metric_match'].shape, dtype=inj_dict['metric_match'].dtype)
	inj_dict['symphony_SNR'] = symphony_match
	inj_dict['mchirp_window'] = mchirp_window
	...
	for i in tqdm(range(templates.shape[0]), desc = desc, leave = True, mininterval = 5, maxinterval = 20):
...
def compute_injections_metric_match(injs, bank, tiling, verbose = True):
	...
	-------
		out_dict: dict
			...
			- ``id_match_list``: list of all the templates within the mchirp window for each injection
