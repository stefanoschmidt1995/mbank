...
To optimize the stochastic method, there has been recently an increasing attention on {\it metric template placing} \cite{Roy:2017oul, 2018cosp...42E2899R, Coogan:2022qxs, manifold_PSU}. Such methods rely on approximating the match between two waveforms with a bilinear form. Despite being approximate, this approach allow for a (much) faster template placing that may overcome some of the major limitations of the standard stochastic placement algorithm.
...
Partially following \cite{manifold_PSU}, we use the function \texttt{SplitTile} to split a tile $\mathcal{T}$ in 3 tiles $\mathcal{T}_{left}$, $\mathcal{T}_{center}$, $\mathcal{T}_{right}$.
