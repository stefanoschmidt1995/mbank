...
\stefano{Write this part again! The matter is not trivial at all and I wanted to keep things as short as possible. An easy way out would be to define $\scalar{h(\theta_1)}{h(\theta_2)}$ in terms of the plus polarization only (that's what we do) but this would result in a very non-standard definition as the PnP review pointed out. How to solve this?\\Also define HM here}
...
%\end{figure}
\textcolor{red}{We need to talk about the conclusions drawn here. I am not sure I completely understand.}
...
As will be shown in Sec.~\ref{sec:placing_accuracy}, the template placing does not seem to be affected by the large inacxcuracy of the metric match.
...
We then randomly draw $50000$ points from the tiling, at each point we compute the metric and its approximation given by the tiling (with and without normalizing flow) and we finally compute the ratio between volume elements. We present our results in Fig.~\ref{fig:tiling_validation}.
\textcolor{red}{We should discuss these conclusions.}
...
Finally, all the histograms are not symmetric around their peak at $0$. This means that the tiling approximation to the metric is more likely to {\it overestimate} the value of the volume element. When it comes to template placing, this translates  into having {\it over-dense} regions.
...
\begin{figure*}[t]
	...
	\caption{Corner plots showing the templates for the three banks described in Tab.~\ref{tab:casestudy_banks}, {\it without} using normalizing flow interpolation. \textcolor{red}{Explain the axes' labels in the caption}}
