"""
Some utilities for mbank, for plotting purposes and for match computation
"""
import numpy as np
...
from .handlers import spin_handler

#TODO: perform a proper injection code:
#	you may use pandas to gather the statistics and you may set an option to compute everything with the metric
#	also, the metric can be used to compute the match properly
...
def ray_compute_injections_match(inj_dict, templates, metric_obj, N_neigh_templates = 10, symphony_match = False):
	"""
	It calls compute_injections_match with ray
	"""
		###
		# Split injections
	...
	for id_, i in enumerate(range(0, inj_dict['match'].shape[0], n_injs_per_job)):
		inj_dict_ray_list.append( _compute_injections_match_ray.remote(i, i+n_injs_per_job, inj_dict,
			templates, metric_obj, N_neigh_templates, symphony_match, id_))
...
@ray.remote
def _compute_injections_match_ray(start_id, end_id, inj_dict, templates, metric_obj, N_neigh_templates = 10, symphony_match = False, worker_id = None):
	...
	for k in inj_dict.keys():
		if isinstance(inj_dict[k], np.ndarray):
			...
			local_dict[k] = None
	return compute_injections_match(local_dict, templates, metric_obj, N_neigh_templates, symphony_match, worker_id)

def compute_injections_match(inj_dict, templates, metric_obj, N_neigh_templates = 10, symphony_match = False, worker_id = None):
	"""
	...
	"""
	assert metric_obj.spin_format == 'm1m2_fullspins_iotaphi', "Wrong spin format given. It must be 'm1m2_fullspins_iotaphi'"

	if N_neigh_templates > templates.shape[0]:
		N_neigh_templates = templates.shape[0]

		#putting injections and templates with the format 'm1m2_fullspins_iotaphi'
	injs = metric_obj.s_handler.get_theta(inj_dict['theta_inj'], 'm1m2_fullspins_iotaphi')
	templates = metric_obj.s_handler.get_theta(templates, 'm1m2_fullspins_iotaphi')
	...
	for i in tqdm(range(injs.shape[0]), desc = desc):
		#FIXME: do it better with some sort of cache!!
		#Take a look at https://pypi.org/project/anycache/
		template_ = templates[inj_dict['id_match_neig'][i,:],:] #(N_neigh_templates, 10)

			#this might be useful for caching the WF computation
		#template_WF = metric_obj.get_WF(template_) #(N_neigh_templates, D)
		#inj_WF = metric_obj.get_WF(inj[i]) #(D,)
		#metric_obj.WF_match(template_WF, inj_WF) #(N_neigh_templates,)
		
			#updating the dict
		true_match = metric_obj.match(template_, injs[i], symphony = symphony_match, overlap = False)
		ids_max = np.argmax(true_match)
		
		#print(i, np.argmax(inj_dict['match_neig'][i,:]), ids_max)
		#print(i, np.argmax(inj_dict['match_neig'][i,:]), ids_max, inj_dict['match'][i],
		#	 true_match[0], "|",
 		#	 inj_dict['match_neig'][i,ids_max],
		#	 true_match[ids_max]) #DEBUG
...
def compute_metric_injections_match_bruteforce(injs, bank,  tiling, N_neigh_templates = 10):
	...
	injs = bank.s_handler.get_theta(injs, bank.spin_format)
	dist_t = []
...
def compute_metric_injections_match(injs, bank, tiling, N_neigh_templates = 10, N_neigh_tiles = 4, tile_id_population = None):
	...
	----------
		injs: 'np.ndarray' (N_injs, 10)
			An array with the injections. They should be have format (N, 10), as output by s_handler.get_BBH_components
...
def plot_tiles_templates(t_obj, templates, spin_format, save_folder, show = False):
	...
	if templates.shape[0] >50000: ids_ = np.random.choice(templates.shape[0], 50000, replace = False)
	...
	try:
		...
		plt.close('all')
		seaborn.pairplot(pd.DataFrame(data=templates[ids_,:], columns = s_handler.labels(spin_format, True)), diag_kind = None, markers = 'o', plot_kws={"s": 3}, corner=True)
	...
	for ax_ in combinations(range(templates.shape[1]), 2):
		...
		currentAxis.scatter(*centers[:,ax_].T, s = 20, marker = 'x', c= 'r', alpha = 1)
		if ax_[0] == 0:
			currentAxis.set_ylabel(s_handler.labels(spin_format, latex = True)[ax_[1]])
		else:
			currentAxis.set_yticks([])
		if ax_[1] == templates.shape[1]-1:
			currentAxis.set_xlabel(s_handler.labels(spin_format, latex = True)[ax_[0]])
		else:
...
def save_injs(filename, injs, GPS_start, GPS_end, time_step, approx, luminosity_distance = 100, f_min = 20.):
		...
		for i, t_inj in enumerate(np.arange(GPS_start, GPS_end, time_step)):
			...
			m1, m2, s1x, s1y, s1z, s2x, s2y, s2z, iota, phi = injs[i]
