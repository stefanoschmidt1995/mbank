...
#@do_profile(follow=[])
def place_random_flow(minimum_match, flow, livepoints, metric, boundaries_checker = None, covering_fraction = 0.01, verbose = True):
	"""
	...
	it = tqdm(dummy_iterator(), desc = bar_str.format(0, 100), disable = not verbose, leave = True)
	
	new_templates = []
	N_tmplts = 0
	
		#Making sure that the livepoints are inside the boundaries
	...
	for _ in it: 
		...
		ids_kill = []
		for i, (l, m) in enumerate(zip(livepoints, metric)):
			diff = proposals - l #(N,D)
			L_t = np.linalg.cholesky(m).astype(dtype) #(D,D)
		...
		new_templates.append(np.array(proposals, dtype = np.float64))
