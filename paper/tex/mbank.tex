...
The rest of this paper is devoted to present and describe our package.
...
\begin{enumerate}
	...
	\item \textit{(Optional)} Training of a normalizing flow model to interpolate the metric whitin each tile and sample from the manifold. 
	\item Placing the templates according to the tiling
\end{enumerate}

Although the template placing is only approximately optimal, our method is order of magnitude faster than the standard approach, as it avoids to generate a large number of waveforms.
...
\begin{equation}\label{eq:stop_tiling}
	0.5\left|\log_{\textrm{10}}\frac{\text{det} M_{right}}{\text{det}M_{left}}\right| > \epsilon,
\end{equation}
...
It is important to realize that a tiling defines also an approximation $p^{\text{tiling}}$ to the uniform probability distribution $p(\theta) \propto \sqrt{|\text{det}M(\theta)|}$\footnote{
...
\end{equation}
It is straightforward to sample from $p^{\text{tiling}}$ using Gibb's sampling: first we randomly choose a tile with a probability proportional to its volume and, second, we randomly draw a point within the chosen tile.
...
\subsection{Normalizing Flow} \label{sec:normalizing_flow}
A normalizing flow model \cite{norm_flow, nflows_paper, 9089305, Papamakarios_thesis} is a machine learning model widely used to reproduce complicated probability distributions.
...
The paramters $W$ of the flow are trained in such a way that:
\begin{equation}
	\mathbf{x} = \phi_W(\theta) \sim \mathcal{N}(\mathbf{x}|0,\mathbf{1}) \;\;\; \text{if} \;\;\;  \theta \sim p(\theta)
\end{equation}
%
In other worlds, a normalizing flow defines an analytical representation of a generic probability distribution $p(\theta)$, obtained by change of variables
...
Sampling from $p^\text{flow}_W$ can then be done exactly by sampling $\mathbf{x} \sim \mathcal{N}(\mathbf{x}|0,\mathbf{1})$ and obtainig $\theta$ from the inverse flow tranformation: $\theta = \phi_W^{-1}(\mathbf{x})$.

The weights $W$ of the flow are set by miniziming the loss function $\mathcal{L}(W)$, defined as the forward Kullback–Leibler (KL) divergence between the true distribution $p(\theta)$ and the one defined by the flow Eq.~\eqref{eq:p_flow}:
\begin{align}
	\mathcal{L}(W) 	&= D_{\text{KL}}[p(\theta) || p^\text{flow}_W(\theta)] \nonumber \\
					&= - \mathbb{E}_{p(\theta)} [\log p^\text{flow}_W] + \text{const.}
\end{align}
The minimization is performed by gradient descent, using empirical samples from the target distribution $p(\theta)$ to provide a Monte-Carlo estimation of the loss function.

The flow transformation $\phi_W$ is built by {\it composing} $n_\text{layers}$ simple (invertible) transformations, each called layer. Of course, depending on the application, a variety of layers is available in literature. We choose to build a layer by concatenating a linear transformation and a Masked Autoregresive Layer \cite{MADE, MAF,MAF_bis} with $n_\text{hidden}$ hidden features.

To deal with a probability distribution bounded in the Rectangle $[\mathbf{m}, \mathbf{M}]$, we employ the following transformation $T_0(\theta)$ as the first layer of the flow:
\begin{equation}
	T_0(\theta) = 0.5 \log \frac{1 + y}{1 - y} \;\;\; \text{with} \;\;\; y = \frac{2\theta - \mathbf{m} - \mathbf{M}}{\mathbf{M}- \mathbf{m}}
\end{equation}
where the fraction above is to be inteneded as elementwise division\footnote{
Note that the inverse $T_0^{-1}$ of the transformation takes a simple form: $\frac{1}{2} [\text{Tanh}(T_0(\theta))(\mathbf{M}- \mathbf{m})+\mathbf{M}+ \mathbf{m}]$, where again the multiplication is intended as elementwise.
}

We train the flow with samples drawn from the tiling Eq.~\eqref{eq:tiling_pdf}.
...
We can then use the trained flow to interpolate the metric whitin each tile:
...
Once a tiling (and possibily a normalizing flow model) is available, we need to place the templates that will be part of the bank.
...
Depending on user's choice, the method can rely either on the metric approximation given by the tiling Eq.~\eqref{eq:metric_tiling}, either by the approximation provided by the tiling+flow Eq.~\eqref{eq:metric_flow}.
As will be shown in Sec.~\ref{sec:bank_generation}, using a normalizing flow for template placing delivers more efficient coverage, providing smaller bank with similar injection recovery.
Whenever a normalizing flow is employed, we will use Eq.~\eqref{eq:p_flow} to sample from the manifold and Eq.~\eqref{eq:metric_flow} to compute the metric; when we do {\it not} utilze a normalizing flow, we use Eq.~\eqref{eq:tiling_pdf} for sampling and Eq.~\eqref{eq:metric_tiling} for computing the metric.
...
Some proposals are randomly drawn as above and they are accepted as templates if their minimum metric distance from {\it all} the previously added templates is greater than  $\sqrt{1-MM}$.
...
In this section we study the performance of different parts of our model. We begin by assessing the quality of the metric approximation to the match Sec.~\ref{sec:metric_accuracy}. In Sec.~\ref{sec:tiling_accuracy}, we evaluate how well the tiling approximates the true metric, with and without using a normalizing flow.
Next in Sec.~\ref{sec:placing_accuracy}, we compare the different placing methods and check their performance on several tiling.
Finally in Sec.~\ref{sec:sbank_comparison}, we compare our code \texttt{mbank} with the state-of-the-art code \texttt{sbank} \cite{sbank} on a selected set of banks. The comparison will be based on bank's size, speed and effectualness.
...
In this section, we evaluate the accuracy of the metric as approximated by the tiling. To measure the discrepancy between the approximated metric $M$ provided by the tiling and the true metric $M_\text{true}$ Eq.~\eqref{eq:metric_expression}, we look at the ratio between volume elements $0.5\log_{\textrm{10}}\frac{\text{det} M}{\text{det}M_\text{true}}$, similar to Eq.~\eqref{eq:stop_tiling}.
...
We then randomly draw $50000$ points from the tiling and at each point we compute the true metric and its approximation given by the tiling (with and without normalizing flow) and we compute the ratio between volume elements. We present our results in Fig.~\ref{fig:tiling_validation}.
...
For all the three banks, templates are placed with the {\it stochastic} method, with a parameter $N_{max}=300$ and as above, we will {\it not} use a normalizing flow model to interpolate the metric. In fig.~\ref{fig:sbank_comparison}, we report the result of an injection study on the three pairs of banks.
...
\begin{figure*}[t]
	...
	\caption{Corner plots showing the templates for the three banks described in table~\ref{tab:casestudy_banks}, {\it without} using normalizing flow interpolation. }
...
\begin{figure*}[t]
	...
	\caption{Corner plots showing the templates for the three banks described in table~\ref{tab:casestudy_banks}, using normalizing flow interpolation.}
...
We generate $75000$ injections randomly drawn from the tiling: the results are reported in fig.~\ref{fig:bank_injections}. We also plot the templates of the banks in Fig.~\ref{fig:bank_scatter} (without normalizing flow interpolation) and in Fig.~\ref{fig:bank_scatter_flow} (with normalizing flow interpolation).
...
The normalizing flow interpolation seems to remove the discontinuities in the template densities. Moreover, it generates a bank with a smaller number of templates, arguably due to a reduction of overcoverage around the boundaries of each tile. This feature is consistent across all the other two banks generated.

The injection recovery (fig.~\ref{fig:bank_injections}) is satisfying for both of precessing banks, with less than $10\%$ of the $75000$ injections performed having a recovery less than the target match $MM = 0.97$ (for the ``no flow" case). The ``flow" case show a consistent histogram.
...
As above, the ``flow" case has less templates with a similar injection recovery.
