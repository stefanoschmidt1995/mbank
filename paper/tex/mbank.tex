...
\sschmidt{Do we mention here the parabolic fit hessian? It's not used afterwards but still may be interesting to discuss}
...
If the metric is close to be degenerate (case 3.), the placing methods (especially {\it uniform} and {\it random}) will provide a poor coverage. This can be easily recognized from a poor metric injection recovery. This happens especially at low masses, where the discrepancy between the first and the last metric eigenvalue can be very large. Problems in this aspect should arise for total mass $M<15 M_{\odot}$.
...
\begin{figure*}[t!]
	...
	\caption{Placing validation: \stefano{Should I add the number of templates in the different cases? I think so...}}
...
\begin{table*}[t!]
	...
	\setlength\extrarowheight{1pt}
	 ...
	 \cdashline{1-4}
	 Aligned spin Lowmass & \begin{tabular}{@{}l@{}} $M\in [10,30]$ \\ $q\in [1,5]$ \\ $s_{1z}, s_{2z}\in [-0.99,0.99]$ \\ $f\in [15,1024] Hz\end{tabular}  &
...
\begin{table*}[t!]
	...
	\setlength\extrarowheight{1pt}
	 ...
	 \begin{tabular}{@{}l@{}} $N_{templates} = 54750$ \\ $N_{tiles} = 33774$ \\ \end{tabular} &
	 	----  \\
	 \cdashline{1-5}
	 Aligned spins HM & \begin{tabular}{@{}l@{}} $M\in [30,50] M_\odot$ \\ $q\in [1,5]$  \\ $\chi \in [-0.99,0.99]$ \\ $f\in [10,1024] Hz \\ \end{tabular}  &
