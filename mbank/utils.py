...
####################################################################################################################
def ray_compute_injections_match(inj_dict, templates, metric_obj, sky_locs = None, mchirp_window = 0.1, symphony_match = False, max_jobs = 8, verbose = True):
	"""
	...
	----------
		...
		sky_locs: :class:`~numpy:numpy.ndarray`
			- shape (N,3)
			Sky localization for the injections. Each row corresponds to longitude, latitude and polarization angle for the injection in ``inj_dict``. Sky localization will be used to compute the antenna pattern: see :func:`get_random_sky_loc` and :func:`get_antenna_patterns` for more information.
			If ``None``, only the plus polarization will be used: this happen only if ``symphony = False``.
	...
	for id_, i in enumerate(range(0, inj_dict['metric_match'].shape[0], n_injs_per_job)):
		inj_dict_ray_list.append( _compute_injections_match_ray.remote(i, i+n_injs_per_job, inj_dict,
			templates, metric_obj, sky_locs, mchirp_window, symphony_match, id_, verbose))
...
@ray.remote
def _compute_injections_match_ray(start_id, end_id, inj_dict, templates, metric_obj, sky_locs = None, mchirp_window = 0.1, symphony_match = False, worker_id = 0, verbose = True):
	"""
	...
	if sky_locs is not None:
		...
		if sky_locs.ndim > 1:
			sky_locs = sky_locs[start_id:end_id]
	
	return compute_injections_match(local_dict, templates, metric_obj, sky_locs, mchirp_window, symphony_match, worker_id, verbose)

def compute_injections_match(inj_dict, templates, metric_obj, sky_locs = None, mchirp_window = 0.1, symphony_match = False, worker_id = None, verbose = True):
	"""
	...
	inj_dict['sky_loc'] = np.asarray(sky_locs)
...
def compute_injections_metric_match(injs, bank, tiling, verbose = True):
	...
	out_dict = {'theta_inj': np.array(bank.var_handler.get_BBH_components(injs, bank.variable_format)).T if injs.shape[1] != 12 else injs ,
				...
				'match': None, 'id_match':  None, 'sky_loc': None,
