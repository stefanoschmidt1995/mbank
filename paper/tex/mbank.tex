...
The exact expression of $p(s = n+h(\theta)|d)$ and $p(s = n|d)$ is obtained under the assumption of stationary Gaussian noise and it depends on the quantities $\scalar{s}{h}$ and $\scalar{h}{h}$, where $\scalar{\cdot}{\cdot}$ a suitable scalar product (see below).
